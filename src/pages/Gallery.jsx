    ...
    <div className="min-h-screen bg-white py-10 ">
      ...
      <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {filteredImages.map((image, index) => (
          ...
          >
            <img src={image.url} alt={image.category} className="w-full h-64 object-cover" />
            <div className="w-full DetailsSec flex items-center gap-3 capitalize">{image.icon} {image.name || "Other"}</div>
          </motion.div>
        ))}
      </div>
